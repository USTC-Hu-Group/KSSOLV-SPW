...
% Spin-restricted and spin-unrestricted case
psi_phi_same = false;
if isempty(phi_in)
	psi_phi_same = true;
	phi_in = psi_in;
...
if ~mol.noncolin
    ...
    Phi2 = cell2mat(Phi3);
    if ~opt.fixip
        [zeta_mu,ind_mu,options]=isdf(conj(Phi2),Psi2,[],options,ispin);
    else
        [zeta_mu,ind_mu,options]=isdf(conj(Phi2),Psi2,opt.ind_mu,options,ispin);
    end
    ...
    if ~mol.lsda
        ...
        rank = options.isdfoptions.rank(ispin);
    end
    ...
    for ik=1:nk_phi
        P_vu(:,:,ik)=Phi3{ik}(ind_mu,:)*(Phi3{ik}(ind_mu,:)'.*(occ{ik}*wks(ik)));
    ...
    if options.aceconv
        ...
        if options.store_tensors
            fprintf('Calculate K_ru and K_vu\n');
            assert(psi_phi_same,'Phi and psi must be same when use Fourier convolution!');
            nkxyz = mol.nkxyz;
            nkx = nkxyz(1);nky = nkxyz(2);nkz = nkxyz(3);
            nk_phi2 = prod(2*nkxyz-1);
