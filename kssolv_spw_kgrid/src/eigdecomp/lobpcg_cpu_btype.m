function [X,lambda,LVEC,RVEC] = lobpcg_cpu_btype(H, X0, prec, tol, maxit, verbose, btype)
%
...
% get size info
%
% Energy Tolerance -> Residual Tolerance
...
% set_btype
if nargin < 7 || isempty(btype)
    btype = ones(ncol, 1); % 默认所有带都需要严格收敛
end
% 为空带设置宽松的残差容差。
% 注意：因为残差是能量误差的开方，所以这里建议放宽到 1e-2 或 1e-3
% empty_tol = max(10 * tol, 1e-3); 
empty_tol = 20.0 * tol;
empty_tol = min(empty_tol, 1.0e-2);

% 构造列向量 tol_array
tol_array = tol * ones(ncol, 1);
tol_array(btype ~= 1) = empty_tol; % 空带赋予宽松标准
...
while (iter <= maxit && nconv < ncol)
  ...
  iconv = find(abs(resnrm) <= tol_array);
  %
  % lock Ritz vectors that satisfy a more stringent convergence tolerance
  %
  tfudge = 1e10;
  % tfudge = 1e-5;
  % lock_threshold = tol_array; 
  % lock_threshold(btype == 1) = tol_array(btype == 1) / 10.0; % 物理带缓冲

  ilock = find(abs(resnrm) <= tol_array/tfudge);
  iact  = find(abs(resnrm) > tol_array/tfudge);
  %
