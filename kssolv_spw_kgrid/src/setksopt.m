...
% spw_coarse_start
options.coarse = 0;
options.scftol_coarse = 1e-2;
options.cgtol_c = options.cgtol;
options.maxcgiter_c = options.maxcgiter;
options.fixcgtol = 0;
options.cbreak = 0;
...
% spw_diag_method
options.maxcgitergpa = options.maxcgiter;
options.spwdavidprec = 0;

% lobpcg empty state speedup
options.lobpcg_btype = 0;

% profile davidson
options.diag_profile = 0;
...
if ~valid
    ...
    if (any(strcmp(lower(value),{'lobpcg';'eigs';'chebyfilt';'omm';'ppcg';'davidson';'davidson2';'davpcg';'cg'})))
       errid = '';
       errmsg = '';
    else
       valid = 0;
       errid = 'MATLAB:funfun:setksopt:eigType';
       errmsg = sprintf('Invalid value for OPTIONS parameter %s: must be one of the following: lobpcg, eigs, chebyfilt, omm, cg\n',field);
    end
end
%-------------------------------------------------------------------------------
