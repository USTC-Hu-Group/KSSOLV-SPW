function [Psi_obj, ev, options] = diag_david_spw(H, Psi_obj, ev, options)
...
% 使用动态字段名读取维度
npw = size(Psi_obj.(fname),1)/npol;
npwx = npw;
% nvec: nbnd of Psi_obj
nvec = size(Psi_obj.(fname),2);
...
% 使用动态字段名赋值
psi(:,1:nvec) = Psi_obj.(fname);

% exert H on inputed Psi_obj
HPsi_obj = H * Psi_obj;
hpsi(:,1:nvec) = 2 * HPsi_obj.(fname); % 提取乘法后的波函数
...
for kter = 1:maxiter
    ...
    % 创建临时对象并使用动态字段名赋值
    Psi_tmp = Psi_obj;
    Psi_tmp.(fname) = psi(:,nb1:nb1+notcnv-1); 
    HPsi_tmp = H * Psi_tmp;
    hpsi(:,nb1:nb1+notcnv-1) = 2 * HPsi_tmp.(fname); 
