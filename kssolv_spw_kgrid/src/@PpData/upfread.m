...
%-------------------------------------------------------------------------
% Read the PP_SPIN_ORB of upf file.
% If there are multiple PP_SPIN_ORB fields exist, it is invalid.
% Only for the full relativistic pseudopotential. 
sobetakey = {'index','lll','jjj'};
sobetainit = {NaN,NaN,NaN};
[spinorbstr,~] = upfreadfield(UPFstr,'PP_SPIN_ORB');

spinorbstr = cell2val(spinorbstr);
if ~isempty(spinorbstr)
    pp.spinorb = struct;
    pp.spinorb = upfinitattrtable(pp.spinorb,sobetakey,...
                                 sobetainit,upf_nbeta);
    for ibeta = 1:upf_nbeta
        [~,attr] = upfreadfield(spinorbstr,...
            ['PP_RELBETA.' num2str(ibeta)]);
    
        pp.spinorb = upfassignattr(pp.spinorb,attr{1},sobetakey,...
                                  sobetainit,ibeta);
    end
end
