...
for itype = 1:ntypes
    ...
    elseif nspin == 2
        ...
        % yinxl 2025.11.05
        % spin-polarizition, the rhog of every atoms devided by its z_valence
        % to get a correct initial magnetic moment
        rho3dg{2}(inz) = rho3dg{2}(inz) + rhog(:,itype).*sum(ccvec.*amag,2)/mol.atoms(itype).venum;
    elseif nspin == 4
        rho3dg{1}(inz) = rho3dg{1}(inz) + rhog(:,itype).*sum(ccvec,2);
        % rho{1} is total density read from pseudopotential files
        ...
        if mol.domag
            ...
            for is = 2:nspin
                rho3dg{is}(inz) = rho3dg{is}(inz) + rhog(:,itype).*sum(ccvec.*amag...
                    .*repmat(angular{is-1},length(inz),1),2)/mol.atoms(itype).venum/2;
                % end yinxl 2025.11.05
            end
        end
    end
end
