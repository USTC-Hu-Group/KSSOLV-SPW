function [mol,H,X,Hprec,varargout] = iterinit_spw(mol,smat_cell,varargin)
...
if isempty(X) 
    %X = genX0(mol,ncol);
    [X, H] = genY0_random_spw(mol,H,smat_cell);
end


Hprec = cell(mol.nkpts,1);
for ik = 1:mol.nkpts
    Gprec = smat_cell{ik};
    Hprec{ik} = genHprecSpw(H,Gprec,ik);
end
